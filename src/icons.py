...
提供 ThemeIcon（明暗自适应静态/SVG 图标）和 GifIcon（GIF 动图图标）。
ThemeIcon 支持单变体自动颜色反转并缓存到 cache/icons/。
"""
...
def _inverted_cache_path(source_path: str, suffix: str) -> str:
    ...
    if not src.exists():
        _logger.warning("图标反转源文件不存在: %s", src)
        return str(src)
    ...
    if not cache_path.exists() or cache_path.stat().st_mtime < src.stat().st_mtime:
        with _CACHE_LOCK:
            if not cache_path.exists() or cache_path.stat().st_mtime < src.stat().st_mtime:
                ...
                _logger.info("生成反转图标缓存: %s -> %s", src.name, cache_path.name)
...
class Icons:
    ...
    静态图标用 FluentIcon；如需自定义 ThemeIcon / GifIcon，替换对应属性即可。
    """
